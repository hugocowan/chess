<template>
  <div class="chessboard">
    <div v-for="square in boardSquares" v-bind:key="square.index" :class="'square ' + square.colour" />
  </div>
</template>
...
export default class ChessBoard extends Vue {
  ...
  public getSquares = ():{ boardSquares: { colour: string; index: number; }[]; } => {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      if (i > 0 && i % 8 === 0) oldcolourA = colourA, colourA = colourB, colourB = oldcolourA;
